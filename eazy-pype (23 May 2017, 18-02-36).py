    ...
    if pipe_params.do_hb:
        ...
        with ProgressBar(nsteps) as bar:
            for i in range(nsteps):
                ...
                if pipe_params.fbad_prior == 'mag':
                    pzarr_hb[GAL] = hb.HBpz(pzarr[:, GAL], zgrid, pzbad[GAL], 
                                            beta_gal, fbad_max = 0.1, fbad_min = 0.0)
                    pzarr_hb[AGN] = hb.HBpz(pzarr[:, AGN], zgrid, pzbad[AGN], 
                                            beta_agn, fbad_max = 0.7, fbad_min = 0.)
                else:
                    pzarr_hb[GAL] = hb.HBpz(pzarr[:, GAL], zgrid, pzbad, 
                                            beta_gal, fbad_max = 0.1, fbad_min = 0.0)
                    pzarr_hb[AGN] = hb.HBpz(pzarr[:, AGN], zgrid, pzbad, 
                                            beta_agn, fbad_max = 0.7, fbad_min = 0.)                
