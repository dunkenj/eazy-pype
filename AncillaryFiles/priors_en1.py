...
if __name__ == '__main__':
    ...
    for ixs, sbset in enumerate([AGN]):
        ...
        if (sbset == GAL).all():
            sbname = 'gal'
            start = np.array([0.525, -0.2435, 0.0352, 1.69169])
            nskip = 1
        ...
        for filt in mags:
            ...
            best, fits, samples, sampler = fitpriors(photom[sbset], filt, z_col, start, 
                                                     nsamples=5000, nthreads=9, nskip=nskip)
