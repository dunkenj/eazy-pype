...
def pz_to_catalog(pz, zgrid):
    ...
    with ProgressBar(len(pz)) as bar:
...
if __name__ == "__main__":
    ...
    for itx, template in enumerate(pipe_params.templates):
        ...
        print('Doing fits...')
        alphas_best, sampler = fitalphas(pz_train, zgrid, zspec_train, alphas_init[itx],
        ...
        plot_path = '{0}/pdf_calibration_{1}.pdf'.format(folder, template)
    ...
    if pipe_params.fbad_prior == 'flat':
        ...
        pzbad = priors.pz(zgrid, mags, *best)
    ...
    betas = np.linspace(1.5, 2.5, 21)
    ci_dists = np.zeros_like(betas)
    
    for ia, beta in enumerate(betas):
        print(beta)
        pzarr_hb = HBpz(pzarr[:,train_index], zgrid, pzbad[train_index], beta=beta)
    ...
    beta_fine = np.linspace(1.5, 2.5, 1001)
    beta_best = beta_fine[np.argmax(fit(beta_fine))]
    
    pzarr_hb = HBpz(pzarr, zgrid, pzbad, beta=beta_best)
    ...
    ci_cosmos, bins = pdf.calc_ci_dist(pzarr[1], zgrid, zspec)
    ci_atlas, bins = pdf.calc_ci_dist(pzarr[2], zgrid, zspec)
    ...
    bright = (photom['I_mag'] >= 12.)*(photom['I_mag'] < 20.)
    faint = (photom['I_mag'] >= 20.)*(photom['I_mag'] < 24.)
    ...
    folder = '{0}/testing/all_specz'.format(pipe_params.working_folder)
    plot_path = '{0}/pdf_calibration_HB.pdf'.format(folder, template)
    Fig.savefig(plot_path, bbox_inches='tight', format='pdf')
    ...
    catpath = '{0}/HB_calibration.cat'.format(folder)
    ...
    def plot_pzs(idn):
        ...
        Ax.plot(zgrid, pzarr[1][idn], label='XMM-COSMOS')
        Ax.plot(zgrid, pzarr[2][idn], label='ATLAS')
